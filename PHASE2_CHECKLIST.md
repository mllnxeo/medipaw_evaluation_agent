...
| Reception | 20 | 0 / 20 | 미착수 — **다음 시작 지점** |
| Followup | 20 | 0 / 100 (표본만 필요) | 미착수 — 표본 추출 기준 논의 필요 |
| Orchestrator | 20 | 0 / 23 | 미착수 |

**다음 세션(또는 다음 배치) 시작 지점**: Reception 라벨링(목표 20/20)
배치 1(케이스 1~5)부터 시작. 이후 순서 제안: Orchestrator →
Followup(표본 추출 방식 논의 — `followup_eval_cases.json`에
`expected_category` 필드가 있는지부터 확인하고 시작).
