...
- `ai/agents/triage/engine.py`의 `match()`/`top_urgency()`/`red_flag_labels()`는
  대화에서 추출한 증상 슬롯(`expected_extracted`)만 보고 판정하며, `gender`
  등 반려동물 프로필 정보는 전혀 참고하지 않음(Phase 2 라벨링 중 발견,
  2026-08-13) — 예: 요도폐색은 임상적으로 수컷 고양이에서 특히 흔한 응급
  상황이지만 현재 엔진은 성별을 판정에 반영하지 않음. 이번 재작업은 골든
  데이터셋·임계값·보안 체계 정비가 목표이고 응급도 판정 규칙 자체의 재설계는
  범위 밖이라, 엔진 로직은 고치지 않고 사실만 기록해둠.
  **[Phase 3 후속 관찰, 2026-08-16]** 같은 요도폐색 골든 케이스로 Chart의
  LLM Judge 보정을 시험하다가 흥미로운 대조를 확인했다 — Triage의 규칙
  기반 엔진(`match()` 등)은 gender를 아예 코드에서 참조하지 않아
  구조적으로 놓치는 반면, `ChartAgent.generate()`는 명시적 지시가 없었는데도
  생성한 SOAP A에 "성별 미상이라 위험도 평가 제한적"이라는 문장을 스스로
  넣어, 성별 정보 부재라는 빈틈을 일반 지식만으로 알아챘다(`chart_judge_calibration.json`
  참고). 규칙 기반 체크는 명시적으로 설계한 것만 잡아내지만, LLM 기반
  체크는 설계자가 놓친 것도 우연히 잡아낼 수 있다는 시사점 — 다만
  "우연히"라는 게 핵심 한계로, 이걸 신뢰할 수 있는 근거로 쓰려면 반복
  측정이 필요하다(Phase 3 표본 8개, 그중 이 관찰은 1건뿐).
- `vet_triage_v3.json`의 TOXIN 섹션이 섭취량·증상 중증도를 반영하지 못하고
  물질명(`substance`)만으로 등급이 고정됨(Phase 2 라벨링 중 발견,
  2026-08-13) — 예: 초콜릿 섭취는 섭취량이 많고 이미 흥분·빈맥 등 증상이
  발현돼도 항상 ORANGE로 고정(`triage_eval_cases.json`의 "초콜릿 섭취" 케이스,
  human_label=FAIL로 표시). 섹션에 `symptom_onset`(무증상/구토/신경증상/출혈)
...
- `case_eval.py`의 rule 체크 전체(2A~2D 등, 특히 2C 예약 겹침)에 대한
  ...
  라벨링(사람 판단) 대상이 아니라, Phase 0에서 버그 1~4에 적용한 방식
  (재현 테스트 먼저 작성 → 수정)으로 다뤄야 하는데, 13번 섹션 Phase
  표에는 이 작업이 별도로 배정돼 있지 않음. 지금 당장 처리하지 않고
  사실만 기록해둠 — 향후 어느 Phase에 배정할지는 별도 논의 필요.
- `DURATION_PROMPT`(`ai/agents/schedule/prompts.py`)의 질환 복잡도 구간
  중 "경련·신경계"만 예시가 없음(Phase 2 라벨링 중 발견, 2026-08-14) —
  같은 구간의 "안구 응급"은 "(각막궤양·포도막염 등)"처럼 예시가 붙어
  있는데 "신경계"는 없어서, 추간판탈출증처럼 신경증상과 정형외과 성격이
  섞인 경계 질환이 +15(신경계) 구간으로 갈지 +0~5(저복잡도) 구간으로
  갈지가 LLM 재량에 맡겨짐(`schedule_eval_cases.json`의 "경추 디스크
  증상" 케이스, human_label=FAIL로 표시). 위 항목들과 유사하게 규칙
  자체의 모호함이지만, gender·TOXIN 건과 달리 **재현성 문제**(같은
  케이스를 반복 평가해도 매번 다른 값이 나올 수 있음)로 이어질 수 있다는
  점이 다름. 프롬프트 자체는 고치지 않고 사실만 기록해둠.
- `schedule_eval_cases.json`의 GREEN 등급 케이스들이 다른 등급 대비
  기대 소요시간 범위 폭(max−min)을 체계적으로 좁게 잡고 있음(Phase 2
  라벨링 중 발견, 2026-08-14) — 20개 전체를 등급별로 집계한 평균 범위
  폭: RED 26.0분(n=5) / ORANGE 25.0분(n=4) / YELLOW 24.0분(n=5) /
  **GREEN 20.8분(n=6)**. 특히 초진 GREEN 케이스는 `DURATION_PROMPT`
  산정 규칙(기본 30 + 초진 5~10 + 체중 5 + 복잡도 0~5)을 그대로 적용하면
  계산 최댓값이 40~50까지 나올 수 있는데, 실제 기대범위 상한은 대부분
  40으로 설정돼 있어 최댓값과 거의 붙거나 넘는 케이스가 다수 발생 —
  **GREEN 6개 케이스 중 5개(83%)**가 이 패턴으로 human_label=FAIL 표시됨
  (연간 예방접종, 중성화 수술 전 검진, 기침 경과 관찰, 구강 위생 상담,
  고양이 정기 검진 — GREEN 중 유일하게 통과한 건 "정기 건강검진" 1건뿐).
  위 DURATION_PROMPT 모호함 건과는 원인·
  방향이 반대 — 저건 "범위가 넓어서 판별력이 약한" 문제(하한이 계산
  최솟값보다 낮음, PHASE2_CHECKLIST.md 참고)이고, 이건 "범위가 좁아서
  정상 케이스도 범위를 벗어나기 쉬운" 문제. 지금 임계값(범위)은 고치지
  않고 사실만 기록 — Phase 6(임계값 확정)에서 실측 데이터로 재검토.
- Reception이 건강/사양관리·부가서비스 관련 질문을 순수 잡담(날씨 등)과
  똑같이 도구 미호출(null)로 처리함(Phase 2 라벨링 중 발견, 2026-08-14)
  — 해당하는 케이스 3건: "강아지 밥은 하루에 몇 번 줘야 해요?"(사양관리),
  "사료 추천해주세요"(사양관리), "강아지 등록은 어떻게 해요?"(관공서
  절차, 단 병원이 대행 서비스를 제공한다면 다를 수 있음). 순수 무관
  질문과 달리 진료 예약·상담 안내나 등록 대행 안내로 이어주는 게
  사용자 경험상 더 나을 수 있음. 지금 도구 체계 기준으로는 케이스
